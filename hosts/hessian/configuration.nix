# Hessian - Raspberry Pi 4B Configuration
# Hardware: Raspberry Pi 4B, microSD storage
# Role: Support services and monitoring worker
{ config, pkgs, inputs, ... }:

{
  imports = [
    # Hardware configuration
    ./hardware-configuration.nix

    # Raspberry Pi profile (includes base, power management, Pi-specific cache)
    # Note: Headless by default (services.xserver.enable = false)
    ../../config/nixos/profiles/raspberry-pi.nix

    # User configuration
    ./main-user.nix
  ];

  # Hostname
  networking.hostName = "hessian";

  # Monitoring role
  myHomelab.services.monitoring.role = "worker";

  # Use latest stable LTS kernel (6.12 as of Nov 2025)
  boot.kernelPackages = pkgs.linuxPackages_6_12;

  # Enable NetworkManager
  networking.networkmanager.enable = true;

  # Bootloader configuration
  boot.loader.grub = {
    enable = true;
    device = "nodev";
  };

  # User management via home-manager
  home-manager.users.beegass = import ./home.nix;

  # Main user configuration
  main-user = {
    enable = true;
    userName = "beegass";
    description = "beegass";
    initialPassword = "12345";
    packages = with pkgs; [
      zsh wget curl git tmux openssh vim neovim htop
    ];
    openssh.authorizedKeys.keys = [
      "ssh-rsa  cardno:15_123_193"
      "ssh-rsa  openpgp:0x5F655FD2 beegass-gpg-key"
    ];
  };

  users.users.beegass.shell = pkgs.zsh;

  # System state version
  system.stateVersion = "24.11";
}
